...
def home():
    ...
    return jsonify({
        ...
        'features': 'Webhook Competitor Extraction (INSTANT)'
    })

@app.route('/webhook/price-change', methods=['POST'])
def handle_price_change():
    """Main webhook endpoint for price changes - WITH INSTANT COMPETITOR EXTRACTION"""
    try:
        ...
        # 🎯 OPTION A: INSTANT WEBHOOK EXTRACTION
        competitor_price = extract_competitor_from_webhook(webhook_data, offer_id)
        
        # Only fallback to scraping if webhook has no competitor data
        if competitor_price is None:
            logger.info("🔄 No competitor data in webhook, falling back to current method")
...
def health():
    ...
    return jsonify({
        ...
        'service': 'Takealot Repricing Engine',
        'version': '1.0.0',
        'feature': 'Webhook Competitor Extraction (INSTANT)'
    })
...
if __name__ == '__main__':
    ...
    logger.info(f"🎯 FEATURE: Webhook Competitor Extraction (INSTANT)")
